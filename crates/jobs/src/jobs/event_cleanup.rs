...
async fn delete_events(
    ...
    debug!(
        log: logger,
        "Retrieving list of events that should be deleted"
    );
    ...
    info!(
        log: logger,
        "Identified {candidate_count} events for deletion"
    );
    ...
    for (event_id, room_id) in candidates {
        ...
        match Event::get_first_for_room(conn, room_id).await {
            ...
            Err(e) => {
                warn!(
                    log: logger,
                    "Failed to retrieve events connected to room: {e}"
                );
            }
        }
    }
    ...
    if deleter_failures > 0 {
        warn!(
            log: logger,
            "{deleter_failures} events could not be deleted due to errors"
        );
    }
...
async fn delete_orphaned_rooms(
    ...
    info!(
        log: logger,
        "Identified {room_count} orphaned rooms for deletion"
    );
    ...
    if deleter_failures > 0 {
        warn!(
            log: logger,
            "{deleter_failures} orphaned rooms could not be deleted due to errors"
        );
    }
    Ok(())
}
