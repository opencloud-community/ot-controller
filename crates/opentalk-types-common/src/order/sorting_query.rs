...
mod utoipa_impls {
    use utoipa::{
        openapi::{
            ...
            Deprecated, Ref, Required,
        },
        IntoParams, ToSchema,
    };

    use super::{Ordering, SortingQuery};
    ...
    impl<'__s, T: ToSchema<'__s>> IntoParams for SortingQuery<T> {
        fn into_params(parameter_in_provider: impl Fn() -> Option<ParameterIn>) -> Vec<Parameter> {
            vec![
                ParameterBuilder::new()
                    ...
                    .schema(Some(Ref::from_schema_name(T::schema().0)))
                    .build(),
                ParameterBuilder::new()
                    .name("order")
                    .required(Required::False)
                    ...
                    .schema(Some(Ref::from_schema_name(Ordering::schema().0)))
                    .build(),
            ]
        }
    }

    fn print_parameter(p: Parameter) {
        ...
        let style = p.style.as_ref().map(|s| match s {
            ...
            ParameterStyle::SpaceDelimited => "space_delimited",
            ParameterStyle::PipeDelimited => "pipe_delimited",
            ParameterStyle::DeepObject => "deep_object",
        });
    ...
    impl<'__s, T: ToSchema<'__s>> SortingQuery<T> {
